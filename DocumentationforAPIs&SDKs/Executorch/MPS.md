...
# Note: `mps_example` script uses by default the MPSPartitioner for ops that are not yet supported by the MPS delegate.
# To turn it off, pass `--no-use_partitioner`.
...
# `Results between ExecuTorch forward pass with MPS backend and PyTorch forward pass for mv3_mps are matching!`
...
[Optional] Generate an [ETRecord](https://docs.pytorch.org/executorch/1.1/etrecord.html) while you’re exporting your model.
...
Run your Program on the ExecuTorch runtime and generate an [ETDump](https://docs.pytorch.org/executorch/1.1/etdump.html).
