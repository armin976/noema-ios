...
See [Partitioner API](https://docs.pytorch.org/executorch/1.1/backends/xnnpack/xnnpack-partitioner.html) for a reference on available partitioner options.

## Quantization

The XNNPACK delegate can also be used as a backend to execute symmetrically quantized models. See [XNNPACK Quantization](https://docs.pytorch.org/executorch/1.1/backends/xnnpack/xnnpack-quantization.html) for more information on available quantization schemes and APIs.

## Runtime Integration

To run the model on-device, use the standard ExecuTorch runtime APIs.

The XNNPACK delegate is included by default in the published Android, iOS, and pip packages. When building from source, pass `-DEXECUTORCH_BUILD_XNNPACK=ON` when configuring the CMake build to compile the XNNPACK backend. See [Running on Device](https://docs.pytorch.org/executorch/1.1/getting-started.html#running-on-device) for more information.
...
* → [Troubleshooting](https://docs.pytorch.org/executorch/1.1/backends/xnnpack/xnnpack-troubleshooting.html) — Debug common issues.
* → [Partitioner API](https://docs.pytorch.org/executorch/1.1/backends/xnnpack/xnnpack-partitioner.html) — Partitioner options and supported operators.
* → [Quantization](https://docs.pytorch.org/executorch/1.1/backends/xnnpack/xnnpack-quantization.html) — Supported quantization schemes.
* → [Architecture and Internals](https://docs.pytorch.org/executorch/1.1/backends/xnnpack/xnnpack-arch-internals.html) — XNNPACK backend internals.
