...
public final class NoemaLlamaClient: @unchecked Sendable {
    ...
    public func load() async throws {
        try await Task.detached { [weak self] in
            ...
            let threads = Int32(max(2, Int(threadsEnv ?? Int32(ProcessInfo.processInfo.processorCount - 2))))
        ...
        return AsyncThrowingStream<String, Error>(bufferingPolicy: .unbounded) { @Sendable continuation in
            ...
            Task { [weak self, input] in
                do {
                    ...
                    let maxTokens = Int(self.contextLength) - 128
                    var aggregated = ""
                    if let paths = imagePaths, !paths.isEmpty {
                        runner.generate(withPrompt: prompt, imagePaths: paths, maxTokens: Int32(maxTokens), onToken: { token in
                        ...
                        runner.generate(withPrompt: prompt, maxTokens: Int32(maxTokens), onToken: { token in
                            continuation.yield(token)
                            aggregated += token
                        }, onDone: {
                            let outPreviewLimit = 20000
