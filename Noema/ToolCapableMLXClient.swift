    ...
    private func buildJSONGrammarPromptFromMessages(_ messages: [ToolChatMessage], tools: [ToolSpec]? = nil) -> String {
        ...
        if let tools, !tools.isEmpty, WebToolGate.isAvailable() {
            if messages.last?.role == "tool" {
                systemContent += "\n\nAnalyze the provided tool results and generate a helpful, natural language response to the user's original question."
            } else {
                ...
                7) Do NOT emit tool calls inside chain-of-thought or <think> sections. If you include a <think> section, ALWAYS close it with </think> and output the <tool_call> (or JSON tool object) only AFTER the final </think>.
                8) Do NOT output tool results yourself and NEVER put results inside <tool_call>; that tag wraps the call JSON only. Do not mix JSON and XML.
...
private func resolveActiveSystemPrompt(from existing: String?) -> String {
    ...
    return SystemPreset.general.text
}
