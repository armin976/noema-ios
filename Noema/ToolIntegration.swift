        ...
        var chatMessage: ToolChatMessage {
            ...
            if let tc = toolCalls {
                mapped = tc.map { c in
                    ...
                    return Noema.ToolCall(id: id, name: name, arguments: argsJSON)
                }
            }
...
struct ToolEnvironment {
    ...
    static func configureForServer() {
        ...
        print("To configure Brave Search, set BRAVE_SEARCH_PROXY_URL=https://your-proxy.example.com at launch or add BraveSearchProxyURL to Info.plist")
    }
    ...
    static func printConfiguration() async {
        ...
        let envProxyURL = (ProcessInfo.processInfo.environment["BRAVE_SEARCH_PROXY_URL"] ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        let bundleProxyURL = (Bundle.main.object(forInfoDictionaryKey: "BraveSearchProxyURL") as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        let proxyURLProvided = !envProxyURL.isEmpty || !bundleProxyURL.isEmpty
        print("- BRAVE_SEARCH_PROXY_URL configured: \(proxyURLProvided)")
