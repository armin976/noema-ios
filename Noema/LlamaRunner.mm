...
static inline bool noema_parse_kv_string(const char * s, ggml_type * out) {
  ...
  // Normalize to uppercase for matching; avoid allocations by manual checks
  ...
  if (equals_ci(s, "F32"))      { *out = GGML_TYPE_F32;   return true; }
  if (equals_ci(s, "F16"))      { *out = GGML_TYPE_F16;   return true; }
  if (equals_ci(s, "Q8_0"))     { *out = GGML_TYPE_Q8_0;  return true; }
  if (equals_ci(s, "Q5_0"))     { *out = GGML_TYPE_Q5_0;  return true; }
  if (equals_ci(s, "Q5_1"))     { *out = GGML_TYPE_Q5_1;  return true; }
  if (equals_ci(s, "Q4_0"))     { *out = GGML_TYPE_Q4_0;  return true; }
  if (equals_ci(s, "Q4_1"))     { *out = GGML_TYPE_Q4_1;  return true; }
  if (equals_ci(s, "IQ4_NL"))   { *out = GGML_TYPE_IQ4_NL;return true; }
...
- (nullable instancetype)initWithModelPath:(NSString *)modelPath
  ...
  mparams.main_gpu = 0;
  ggml_backend_dev_t cpu_devices[2];
  if (nGpu <= 0) {
  ...
  // --- Separate KV cache quantization with safe defaults ---
  // NOTE: V-cache quantization is disabled; always use F16 for V.
  NOEMAKVCacheConfig cfg = self.kvConfig;
  ggml_type kType = GGML_TYPE_F16;
  ggml_type vType = GGML_TYPE_F16;
  // Allow environment to override K when provided by the app
  const char *env_k = getenv("LLAMA_K_QUANT");
  ggml_type parsed;
  if (env_k && noema_parse_kv_string(env_k, &parsed)) { kType = parsed; }
  // If caller explicitly enabled KV config, that takes precedence over env (K only)
  if (cfg.enabled) {
    kType = noema_map_kv_type(cfg.typeK);
  }

#if defined(LLAMA_API_VERSION) && (LLAMA_API_VERSION >= 1400)
  cparams.type_k = kType;
  cparams.type_v = vType;
#else
  // Older llama.cpp only supports one kv_type for both K and V.
  // Use the stricter (larger precision) type to be safe and warn.
  ggml_type merged = (kType == GGML_TYPE_F32 || vType == GGML_TYPE_F32) ? GGML_TYPE_F32 :
                     ...
                     GGML_TYPE_Q4_0;
#if defined(LLAMA_CONTEXT_PARAMS_HAS_KV_TYPE)
  cparams.kv_type = merged;
#else
  // Fallback for very old builds: set type_k/type_v if present, else ignore.
  cparams.type_k = merged;
  cparams.type_v = merged;
#endif
  if (_verbose) {
    NSLog(@"[Noema] Warning: llama.cpp too old for separate K/V cache types. Using merged kv_type=%s.", noema_kv_type_name(merged));
  }
#endif
