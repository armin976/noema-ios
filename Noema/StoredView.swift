            ...
            if modelManager.downloadedModels.isEmpty {
                VStack(spacing: 12) {
                    ...
                    Button {
                        ...
                        Label("Explore Models", systemImage: "sparkles")
                    }
                    .buttonStyle(.borderedProminent)
                    Button {
        ...
        Section("Remote Backends") {
            ...
            .onDelete { indexSet in
                for index in indexSet.sorted(by: >) {
                    guard modelManager.remoteBackends.indices.contains(index) else { continue }
                    let id = modelManager.remoteBackends[index].id
                    modelManager.deleteRemoteBackend(id: id)
                }
            }
        }
    }
