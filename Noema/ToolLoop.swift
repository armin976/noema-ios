...
public final class ToolLoop {
    ...
    public func runWithXMLGrammar(messages: inout [ToolChatMessage]) async throws -> String {
        ...
        for turn in 0..<maxToolTurns {
            ...
            if let toolCall = try? parseXMLToolCall(response.trimmingCharacters(in: .whitespacesAndNewlines)) {
            ...
            if let toolCall = try? parseSimpleToolCall(response.trimmingCharacters(in: .whitespacesAndNewlines)) {
