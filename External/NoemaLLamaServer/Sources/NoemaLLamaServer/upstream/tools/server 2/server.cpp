...
// Exposed for Noema's in-process embedding so the app can request a graceful
// shutdown from outside main().
std::function<void(int)> shutdown_handler;
static std::atomic_flag is_terminating = ATOMIC_FLAG_INIT;

// Bridge callbacks implemented in bridge/server_bridge.mm.
...
int main(int argc, char ** argv) {
    ...
    if (!ctx_http.init(params)) {
        noema_llama_server_report_error("Failed to initialize HTTP server.");
    ...
    if (is_router_server) {
        ...
        try {
            ...
            const std::string message = std::string("Failed to initialize router models: ") + e.what();
            noema_llama_server_report_error(message.c_str());
    ...
    if (is_router_server) {
        ...
        if (!ctx_http.start()) {
            clean_up();
            noema_llama_server_report_error("Failed to start HTTP server.");
        ...
        noema_llama_server_report_load_progress(0.0f);
        params.load_progress_callback = [](float progress, void * user_data) {
            GGML_UNUSED(user_data);
            noema_llama_server_report_load_progress(progress);
            return true;
        };
        ...
        if (!ctx_server.load_model(params)) {
            ...
            noema_llama_server_report_error("Failed to load model in loopback server.");
            LOG_ERR("%s: exiting due to model loading error\n", __func__);
            return 1;
        }

        routes.update_meta(ctx_server);
        noema_llama_server_report_load_progress(1.0f);
        ctx_http.is_ready.store(true);
        noema_llama_server_report_http_ready();
