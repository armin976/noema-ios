...
> **Remote access**
>
> Ollama listens on `localhost` by default. To reach it from another device (such as an iPhone or iPad running Noema), start Ollama with `OLLAMA_HOST=0.0.0.0` (or `ollama serve --host 0.0.0.0`) and point the base URL in Noema to your computer's LAN IP address, e.g. `http://192.168.0.10:11434`.
